...
const TIME_PER_QUESTION = 20;      // s
const FEEDBACK_DELAY    = 700;     // ms
...
function render() {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => cont.classList.remove('opacity-0'))
  );
  ...
  q.answers.forEach((ans, i) => {
    ...
    b.dataset.i  = i;
    b.className  =
      'w-full text-left px-4 py-3 border rounded-lg bg-white hover:bg-gray-50 transition transform';
    b.textContent = ans;
    ul.appendChild(b);
  });
  ...
  st.tId = setInterval(() => {
    st.rem--;
    if (st.rem === 0) { clearInterval(st.tId); handle(-1); }
    updateTimerUI();
  }, 1000);
}
...
function updateTimerUI() {
  ...
  const percent = (st.rem / TIME_PER_QUESTION) * 100;
...
function handle(choice) {
  ...
  // highlight wrong + shake
  if (choice !== q.correct && choice !== -1) {
    btns[choice].classList.add(
      'bg-red-500',
